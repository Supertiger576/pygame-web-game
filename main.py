...
def reset_game():
    global player_rect, player_y_velocity, score, game_started, game_over, pipe1
...
async def main():
    global player_y_velocity, game_started, game_over, score, notjumpedyet, player_visible, pipe1_visible, pipe1, pipe2
    running = True
    while running:
        ...
        if not game_over and game_started:
            ...
            if pipe1.right < 0:
                new_height = random.randint(300, 500) 
                pipe1 = make_extra_rect(windowWidth, new_height, 50, 500)
            if pipe2.right < 0:
                pipe2 = make_extra_rect(windowWidth, 0, 50, random.randint(100, 250))
        ...
        if pipe1_visible:
            pygame.draw.rect(screen, (0, 255, 255), pipe1)
        if pipe1_visible:
            pygame.draw.rect(screen, (0, 255, 255), pipe2)

        if not game_started and notjumpedyet:
            jump_surface = font.render("Press Space to Start Jumping!", True, (0, 0, 0))
            screen.blit(jump_surface, (windowWidth // 2 - 200, windowHeight // 2 - 50))
